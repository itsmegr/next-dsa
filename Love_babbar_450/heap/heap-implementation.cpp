...
//explicitly functions for heap
int size = 0;
void percolateUp(vector<int> ar, int start)
{
    int parent = (start - 1) / 2;
    if (parent >= 0 && ar[start] > ar[parent])
    {
        swap(ar[start], ar[parent]);
        percolateUp(ar, parent);
    }
}
void percolateDown(vector<int> ar, int start)
{
    int l = 2 * start + 1;
    int r = 2 * start + 2;
    int largest = start;
    ...
    {
        swap(ar[start], ar[largest]);
        percolateDown(ar,largest);
    }
}
void insert(vector<int> ar,int key)
{
    size++;
    ar.push_back(key);
    percolateUp(size - 1);
